...
- **用纱估算**：成品幅宽/长度、经纬缩率、上机回丝、线密度（tex）→ 总经根数、
  投纬数、经纬纱总长、总质量及分色用量。
- **整经与穿筘**：按幅宽×经密推算全幅经纱；设置筘密（齿/cm 或齿/英寸）、每齿
  根数模式、边纱规则与每束上限后，自动生成穿筘分配与分束方案（保持色序连续），
  列出每束各色根数、单根长度与总用量；密度偏差、筘齿不匀、束界切色、边纱落位
  即时标出。逐根上机视图可缩放，键盘/按钮勾选整经、穿综、穿筘进度，可回退、
  续接并定位回原草图；计划与进度随项目版本保存，可打印按束汇总的操作单。
...
static/js/
  ...
  warp.js               整经/穿筘规划计算（筘齿分配、分束、检查）
  warpview.js           逐根上机视图（可缩放路径与进度勾选）
  warpsheet.js          整经操作单（按束汇总与上机次序）
  api.js / utils.js
tests/
  warp.test.mjs         整经/穿筘计算回归测试：node tests/warp.test.mjs
```
