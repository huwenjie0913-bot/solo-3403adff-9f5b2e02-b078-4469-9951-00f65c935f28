...
- **提综 → 踏板方案转换与优化**（右侧「踏综」标签或提综表上方按钮打开转换台）：
  读取当前草稿（或 WIF 导入）的逐纬提综状态，在指定踏板数、每纬同时踩踏上限、
  必须保留/禁止的栓结格（绿/红标记）以及锁定整列踏板的约束下，回溯搜索能逐纬
  精确复现提综组合的栓结与踏序候选；升综/降综分别沿用现有推演。无解时列出发生
  冲突的纬与约束原因（并可点击定位）。候选按「使用踏板数 → 多踏板纬数 →
  总踩踏次数 → 相对当前栓结改动量」排序，卡片可点选，预览栓结矩阵、踏序、
  逐纬提综校验与组织全图差异。应用候选时自动把原草稿存为“转换前快照”版本、
  再把新方案写为当前草稿并另存新版本；此后 WIF 导出、版本比较与打印工艺单
  均使用选定方案。
...
static/js/
  ...
  treadle.js            提综→栓结/踏序候选搜索（约束回溯、冲突报告）
  treadleview.js        踏板方案转换台（约束编辑、候选、Canvas 预览）
...
tests/
  ...
  treadle.test.mjs      提综→踏板转换搜索回归测试：node tests/treadle.test.mjs
```
